...
const ResourceGuard = require('./layers/resource_guard');

// ReDoS保护：带超时检测的安全正则测试
const REGEX_TIMEOUT_MS = 1000; // 1秒超时

function safeRegexTest(pattern, input) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), REGEX_TIMEOUT_MS);
  
  try {
    const regex = new RegExp(pattern, 'gi');
    const result = regex.test(input);
    clearTimeout(timeout);
    return { safe: false, matched: result };
  } catch (e) {
    clearTimeout(timeout);
    // 正则错误（如灾难性回溯）也视为不安全但跳过
    return { safe: true, matched: false, error: e.message };
  }
}
...
class Detector {
  constructor(skillPath = __dirname) {
    ...
    // ResourceGuard是函数模块，直接使用
    this.rg = ResourceGuard;
  ...
  scan(input, options = {}) {
    ...
    // Resource Guard 层检测（URL/资源访问安全）
    if (this.rg && (layer === 'all' || layer === 'resource')) {
      // 自动检测URL并验证
      const urlPattern = /^https?:\/\//i;
      if (urlPattern.test(input)) {
        const rgResult = this.rg.validateURL(input);
        if (!rgResult.safe) {
          results.push({ layer: 'resource_guard', threats: rgResult.threats, confidence: 0.95 });
        }
      }
      // 也检测可能的URL模式
      const potentialUrlMatch = input.match(/(https?:\/\/[^\s'"]+)/gi);
      if (potentialUrlMatch) {
        for (const url of potentialUrlMatch) {
          const rgResult = this.rg.validateURL(url);
          if (!rgResult.safe) {
            results.push({ layer: 'resource_guard', threats: rgResult.threats, confidence: 0.9 });
          }
        }
      }
    }
  ...
  _detectSupplyChain(code) {
    ...
    for (const pattern of ruleSet.patterns || []) {
      ...
      } catch (e) { console.error("[奇点造物-Genesisix] Detection error in layer: " + e.message); }
    }
...
module.exports.ResourceGuard = ResourceGuard;

// 示例用法
if (require.main === module) {
  console.log('奇点造物-Genesisix Detector Demo\n');
