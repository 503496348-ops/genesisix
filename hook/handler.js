/**
 * 奇点造物-Genesisix Input Hook / 奇点造物-Genesisix 输入拦截器
...
function getBlockedMessages(lang, result) {
  ...
  const messages = [
    isZh ? '🛡️ 输入已被 奇点造物-Genesisix 拦截' : '🛡️ Input blocked by 奇点造物-Genesisix',
    '',
    isZh ? '威胁类型: ' : 'Threats: ',
    threatTypes.join(', '),
    '',
...
function initDetector() {
  ...
  try {
    ...
    if (fs.existsSync(detectorPath)) {
      ...
      console.log('[奇点造物-Genesisix] Detector initialized');
    }
  } catch (e) {
    console.error('[奇点造物-Genesisix] Failed to load detector:', e.message);
  }
...
function scanAndBlock(input, event) {
  ...
  if (!result.safe && result.confidence >= 0.6) {
    console.log('[奇点造物-Genesisix] Blocked input:', {
...
async function handler(event) {
  ...
  if (!d) {
    console.warn('[奇点造物-Genesisix] Detector not available, allowing input');
    return;
  }
